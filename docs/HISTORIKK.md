...
*(Bestilling 7 — lys-oppgradering 24V, bestilt & betalt 5. juli 2026, 795,72 kr — ble **komplett mottatt 7. august 2026**. Detaljtabell står i skillen + `mottaksliste.md`.)*
...
**7. august 2026 — ✅ B7 KOMPLETT (7 av 7), ingen bestilling har utestående leveranser.** To pakker samme dag: **24V→5V buck 5A ×2** (lovet ~30. juli) og **FR120N MOSFET ×4** (lovet ~13. aug — kom nesten en uke tidlig, altså ikke sist likevel). Bucken har skrueklemmer + to sylindre (induktor + elektrolyttkondensator — normal step-down-topologi; det er ikke to spoler). Konsekvenser: (1) **hele 24V-lyskjeden kan nå bygges** — ingenting venter lenger på posten, prosjektet er kun begrenset av fase 3-designet og byggetid; (2) tre byggesjekker aktiveres: lodde skrueklemmene på FR120N (byggesett som i B1, der 1 av 4 ble skrapet av for mye varme), måle buck-utgangen (~5V) *før* ESP32 kobles på — den nye bucken kan i motsetning til den gamle ha trim-potmeter — og strømmåling ~1,0 A/bar @24V med UT131D (siste harde spec-sjekk på barene); (3) **badge-synk-regelen mellom `oversikt.html` og `mottaksliste.md` kan avvikles** som planlagt ved komplett B7 — Komponenter-fanen fryses med peker til mottakslista, `lager.md` blir eneste levende liste; (4) 12V-utstyret (12V-adaptere, fast-5V-buck, LR7843 ×3, phyto-stripa 5 m) går fra «bench-strøm i påvente av deler» til ren reserve/lek. Den fysiske risikoen består: 24V- og 12V-adapterne har identisk 5,5×2,1mm-plugg, og 24V ødelegger gammel buck / LR7843 / phyto-stripa umiddelbart → merk pluggen.
...
- **PPFD-standard korrigert til måling (2026-08-07):** `settings.ts` → `LYS_VARIANTER.bar24.standardPpfd` **250 → 213**. 250 var produsentens spec-midtpunkt (250–350); Photone-måleserien 28. juli ga ~208 PPFD under én bar og ~213 i snitt over flaten med to barer @10 cm. Specen var altså ~17 % for optimistisk, og alle DLI-tall/lysanbefalinger i appen overvurderte lyset tilsvarende. Konsekvens for dyrkingen: fotoperioden må til **16–18 t** (ikke 13 t som i den opprinnelige oppstartsplanen) for DLI 12–17, og hengehøyden til **~10 cm**. NB: har brukeren kalibrert manuelt, ligger verdien i localStorage og overstyrer denne standarden.
- **Badge-synk-regelen AVVIKLET (2026-08-07):** B7 er komplett mottatt → bestillings-æraen er over. Komponenter-fanen i `oversikt.html` er frosset med en synlig notis + peker til `mottaksliste.md`, og `lager.md` er fra nå eneste levende liste over hva som eies. Bygg-fanen ble samtidig skrevet om fra 12V til 24V (trinn 0 og 2), siden alle deler og tilkoblingsdetaljer nå er kjent.
