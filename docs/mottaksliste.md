...
*Sist oppdatert: 2026-08-07 — **✅ ALLE 7 BESTILLINGER KOMPLETT MOTTATT.** B7 ble ferdig 7. august med buck ×2 og FR120N ×4. Photone-målingen av barene er gjort (28. juli, ~208 PPFD @10 cm/bar — gyldig, ingen reklamasjonsgrunn). **Ingenting er i posten, og ingen leveranse blokkerer byggingen.** Gjenstående sjekker hører til byggesteget: lodde FR120N-klemmer, måle buck-utgang (~5V) før ESP32, og strømmåling ~1,0 A/bar @24V.*
...
Vekstlys-byttet: magenta 12V-strip → hvite 24V Samsung LM281B-barer (~10× lys, fikser kamera-magenta). Uavhengig verifisert 5. juli (research-doc `2026-07-05-vekstlys-research-oppdrag.md`). 7 varer / 7 del-ordrer: subtotal 546,36 + frakt 90,13 + mva 159,23 = **795,72 kr**. Leveringene kom spredt (25. juli–7. aug). **Komplett 7. august 2026.**

**Mottaksstatus 7. august: ✅ B7 KOMPLETT — 7 av 7 mottatt.** Buck ×2 og FR120N ×4 kom samme dag (7. aug); FR120N-ene var nesten en uke tidlig. **Alle delene til 24V-lyskjeden er nå på lager** — det som gjenstår er bygging: lodde skrueklemmer på FR120N, måle buck-utgangen (~5V) før ESP32, og strømmåling ~1,0 A/bar. Delene legges rett på lager uten inspeksjon (Markus' valg — antatt OK); det som må sjekkes står per rad som *«verifiseres ved bruk»* og hører til byggesteget, ikke mottaket. **Unntaket er barene:** Photone-målingen har en frist (Buyer Protection) og må gjøres nå.

> 💡 **Avgjort 25. juli: potte 1 bygges som 24V fra start** — 12V-mellomsteget er droppet. **Per 7. aug er ingenting lenger blokkert av leveranser** — hele 24V-kjeden (adapter → sikring → FR120N → barer, + buck → 5V → ESP32) kan kobles så snart FR120N-klemmene er loddet.
>
...
| 📦 | FR120N MOSFET-modul 100V 9.4A, opto-isolert (TZT teng Official) | ×4 | 4,05 kr/stk · ordre `3074999822094751` · **Kom 7. aug** (nesten en uke FØR lovet 13. aug). Bytter LR7843 (30V for tett på 24V). 4 stk = potte 1 + potte 2 + 2 reserve. Samme byggesett-type som B1 → skrueklemmer må loddes |
| 📦 | 24V/12V→5V **5A** buck 25W (WAVGAT Official) | ×2 | 16,31 kr/stk · ordre `3074999822154751` · **Kom 7. aug** (lovet ~30. juli). Ny 5V-kilde (gammel buck er 12V-inn, tåler ikke 24V). 5A ut = rikelig for ESP32+CAM. Én per potte. Kortet har skrueklemmer + to sylindre (induktor-spole + elektrolyttkondensator — normalt for en step-down). *Verifiseres ved bruk:* **mål ~5V med multimeter FØR ESP32 kobles på**; kortet kan ha trim-potmeter (justerbar type) — da må utgangen stilles inn før bruk, i motsetning til den gamle faste 5V-bucken |
