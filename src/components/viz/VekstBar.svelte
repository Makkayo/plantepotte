<script lang="ts">
  ...
  let {
    ...
    klar = false,
    size = 'mini',
  }: { prosent: number; klar?: boolean; size?: 'mini' | 'full' } = $props();
...
<div class="relative w-full" role="img" aria-label="Vekst {p}%{klar ? ', klar til høsting' : ''}">
  ...
  <div
    class="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 rounded-full transition-[left] duration-700 {dot}"
