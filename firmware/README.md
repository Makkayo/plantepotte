...
|-----|-----|:---:|
| `config.py` | Innstillinger — **her endrer du WiFi-navn og passord** | ✅ |
...
5. **Endre `config.py`**: sett `WIFI_SSID` og `WIFI_PASS` (kun 2,4 GHz-nett).
...
- ⏰ **Riktig lys-timer krever internett-tid (NTP).** Uten WiFi ved oppstart vet
  ikke ESP32 klokka, og timeren blir feil. Den henter tid på nytt ved gjenoppkobling.
